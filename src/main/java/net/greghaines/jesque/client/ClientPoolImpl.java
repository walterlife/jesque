...
public class ClientPoolImpl extends AbstractClient {
    ...
    @Override
    protected void doBulkEnqueue(final String queue, final List<String> jobsSerialized, final int batchSize) throws Exception {
        PoolUtils.doWorkInPool(this.jedisPool, new PoolWork<Jedis, Void>() {
            /**
             * {@inheritDoc}
             */
            @Override
            public Void doWork(final Jedis jedis) {
                doBulkEnqueue(jedis,getNamespace(),queue,jobsSerialized,batchSize);
                return null;
            }
        });
    }
